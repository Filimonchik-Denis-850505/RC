...
namespace form {
	...
	public:
		...
		{
			None.loadFromFile("font/Roboto.ttf");			//передаем нашему шрифту файл шрифта
	...
	public:
		Button(double _x = 0, double _y = 0, double _width = 230, double _height = 50, string _text = "Connect")
		{
			None.loadFromFile("font/Roboto.ttf");			//передаем нашему шрифту файл шрифта
			...
			TXT.setCharacterSize(20); 					// в пикселях, а не точках!
			TXT.setFillColor(Color::Black);					// устанавливаем цвет выводимого текста
			TXT.setPosition(555, 312);						// позиция текста

			BOX.setSize(Vector2f(width, height));		// размер кнопки
			BOX.setPosition(x, y);						// координаты кнопки
			BOX.setFillColor(sf::Color(82, 103, 177));	// цвет кнопки
	...
	public:
		Input(double _x = 0, double _y = 0, double _width = 150, double _height = 30, string _text = "")
		{
			...
			BOX.setFillColor(Color::Yellow);	// цвет кнопки
		}
